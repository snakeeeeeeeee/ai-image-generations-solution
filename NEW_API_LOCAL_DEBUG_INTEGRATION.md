# new-api 本地联调 image-handle 异步生图对接文档

本文档给 new-api 本地开发联调用。完整协议说明见 `NEW_API_IMAGE_HANDLE_INTEGRATION.md`。

本次只对接异步任务链路。旧同步接口 `/v1/images/generations`、`/v1/images/edits` 仍保持原逻辑，不需要 new-api 为同步接口做额外改造。
...
- 图片结果只返回 R2 URL，不把上游大 base64 回传给 new-api。
...
- `IMAGE_HANDLE_API_KEY` 对应 image-handle 的 `PROVIDER_API_KEYS`，用于 new-api 调 `POST /v1/image/tasks`。
...
- 成功 callback 只包含 R2 URL，不包含真实 `b64_json`。
