...
负载均衡
  ...
  -> 共享 Redis/BullMQ
  -> image-worker x M
  -> 上游 new-api/OpenAI/xAI
  -> R2
  -> image-notifier x K
  -> 回调通知 new-api
```

API、任务处理进程、回调投递进程都是无状态进程。新机器只要运行同一个镜像，并使用同一套 `.env` 指向共享的 `POSTGRES_URL`、`REDIS_URL`、R2 和上游配置，就可以加入处理。
...
幂等键是 `provider_api_key + client_task_id`。重复提交会返回已有的 `provider_task_id`，不会重复入队，也不会重复生图。
