...
`provider_api_key` 需要配置在 image-handle 的 `PROVIDER_API_KEYS` 中。多个 key 可用英文逗号分隔，便于轮换。
...
| `metadata.channel_id` | 建议 | 用于按 `provider + model + channel_id` 做限速维度。 |
...
```text
provider_api_key + client_task_id
```
...
```env
PROVIDER_API_KEYS=provider-key-from-new-api
POSTGRES_URL=postgres://...
REDIS_URL=redis://...
NEW_API_BASE_URL=http://new-api-internal:3000
UPSTREAM_API_KEY=upstream-key-used-by-worker
